...
def _filter_specs(specs: dict) -> dict:
    ...
    for k, v in specs.items():
        ...
        val = str(v).strip()
        if key in {"scrape error", "scrape warning"}:
            continue
        if re.search(r"could not load|access denied", val, re.I):
            continue
        filtered[k] = v
    return filtered
...
def generate_ebay_title(raw_title: str, brand: str, specs: dict) -> str:
    ...
    safe_specs = _filter_specs(specs)
    spec_str = ", ".join(f"{k}: {v}" for k, v in list(safe_specs.items())[:5])
    ...
    for k, v in list(safe_specs.items())[:3]:
        candidate = f"{v}"
...
def generate_ebay_description(
    ...
    safe_specs = _filter_specs(specs)
    spec_lines = "\n".join(f"- {k}: {v}" for k, v in safe_specs.items())
    ...
    spec_rows = "".join(
        ...
        for k, v in safe_specs.items()
    ) if safe_specs else "<tr><td colspan='2' style='padding:8px'>See product images for details</td></tr>"
